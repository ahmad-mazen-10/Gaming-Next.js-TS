...
function GamesSlider({ games, title, slidePreview = 4, big, screenBig }: GamesSliderProps) {
  return (
    <div className="mt-14 flex flex-col gap-6">
      ...
      <SwiperCards
        ...
        items={games.map((game) => ({
          card: big ? (
            <div key={game.id} className="flex items-center overflow-hidden bg-[#333839] rounded-sxl">
              <div className="flex w-[60%] flex-col px-6 items-start">
                <h1 className="text-2xl font-semibold border-b-2 w-full border-neutral-100 pb-3 text-white">
                  {game.name}
                </h1>
              ...
              <div className="w-[60%] relative">
                <Image
                  ...
                  alt={game.name}
                  fill
                  className="group-hover:scale-125 group-hover:rotate-12 duration-200 object-cover"
                />
              </div>
            </div>
          ) : (
