...
function Search() {
    ...
    return (
        <div ref={outsideREF} className=" w-full flex relative  group items-center gap-2 justify-between px-4 border border-input  rounded-xl md:w-[40%] bg-main">
            <input value={query}onChange={(e) => setQuery(e.target.value)} className="py-2 text-base w-full bg-transparent text-gray-50 border-none outline-none active:outline-none ring-0 placeholder:text-gray-400" />

            <div className="flex items-center gap-2">
                <SearchIcon className='w-5 h-5 cursor-pointer group-hover duration-150 hover:text-rose-400' />
                <XIcon onClick={() => { setQuery(''); setSearch(''); }} />
            </div>
